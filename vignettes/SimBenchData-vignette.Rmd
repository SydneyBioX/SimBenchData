...
Information about each dataset such as its description and source URL can be found in the metadata and the exact processing step can be found under the `inst/scripts` directory.     

Additional details on each dataset can be explored using the command `showAllData()`. The information on the first three datasets is shown as an example.   
